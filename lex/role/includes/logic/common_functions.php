<?php
  ...
  function isOfficer($user_id) {
    global $conn;
    $sql = "SELECT * FROM users WHERE id=? AND role_id=2 LIMIT 1";
    $user = getSingleRecord($sql, 'i', [$user_id]); // get single user from database
    if (!empty($user)) {
      return true;
    } else {
   ...
   function isUser($user_id) {
    ...
    function isSuper($user_id) {
    global $conn;
    $sql = "SELECT * FROM users WHERE id=? AND role_id=0 LIMIT 1";
    $user = getSingleRecord($sql, 'i', [$user_id]); // get single user from database
    if (!empty($user)) {
      return true;
    } else {
 ...
 function isDirector($user_id) {
    global $conn;
    $sql = "SELECT * FROM users WHERE id=? AND role_id=5 LIMIT 1";
    $user = getSingleRecord($sql, 'i', [$user_id]); // get single user from database
    if (!empty($user)) {
      return true;
    } else {
  ...
  function loginById($user_id) {
    global $conn;
    $sql = "SELECT u.id, u.role_id, u.username,u.email ,r.name as role FROM users u LEFT JOIN roles r ON u.role_id=r.id WHERE u.id=? LIMIT 1";
    ...
    if (!empty($user)) {
      ...
      if (isSuper($user_id)) {
        ...
        echo '<h1 style="color: #0F0;">Login Successful. </h1>';
      ...
      if (isAdmin($user_id)) {
        ...
        $_SESSION['userPermissions'] = $userPermissions;
        echo '<h1 style="color: #0F0;">Login Successful. </h1>';
		echo '<meta http-equiv="refresh" content="2;URL=../officer/home">';
        ...
        $_SESSION['userPermissions'] = $userPermissions;
        echo '<h1 style="color: #0F0;">Login Successful. </h1>';
		echo '<meta http-equiv="refresh" content="2;URL=../employee/home">';	
	  }else if(isDirector($user_id)){
        $permissionsSql = "SELECT p.name as permission_name FROM permissions as p
                            JOIN permission_role as pr ON p.id=pr.permission_id
                            WHERE pr.role_id=?";
        $userPermissions = getMultipleRecords($permissionsSql, "i", [$user['role_id']]);
        $_SESSION['userPermissions'] = $userPermissions;
        echo '<h1 style="color: #0F0;">Login Successful. </h1>';
    echo '<meta http-equiv="refresh" content="2;URL=../director/index">';  
    }else {
