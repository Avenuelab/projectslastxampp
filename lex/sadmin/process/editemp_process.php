...
if($role>1){
	...
	if(!empty($_POST)){ 
		...
		//$memdt= date('Ymd',strtotime($subscr));
			$pref=254;
		$phone=$pref.$phone;
	...
	$empSQL="UPDATE employee SET  name='$name', gender='$gender', dob=$memdob, residence='$residence', prevemployer='$prevemployer', phone='$phone', idno=$natid, pin='$pin', narration='$narration', marital='$marital', pobox='$pobox',locationid='$loc',designation='$designation', nhif='$nssf', nssf='$nhif',email='$email' WHERE empid='$empid'";
	...
	$empSQL="UPDATE employee SET  name='$name', gender='$gender', dob=$memdob, residence='$residence', prevemployer='$prevemployer', phone='$phone', idno=$natid, pin='$pin', narration='$narration', marital='$marital', pobox='$pobox',locationid='$loc',designation='$designation', nhif='$nssf', nssf='$nhif',picpath='$dbname',email='$email' WHERE empid='$empid'";
						$query=mysqli_query($conn,$empSQL);
				...
				$temp = explode(".",$target_file);
				$newfilename = $target_dir . $newName. '.' . end($temp);
				$iddbname="../editpic/" . $newName. '.' . end($temp);
				$uploadOk = 1;
				$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
				// Check if image file is a actual image or fake image
						...
						if (file_exists($newfilename)) {
						   //echo "Sorry, file already exists.";
							...
							if ((move_uploaded_file($_FILES["uploadid"]["tmp_name"],$newfilename))) {
								echo "New profile pic  ".$newfilename."has been uploaded.";
					echo '<meta http-equiv="refresh" content="2;URL=../editempprofile.php?edit_id=' . $empid . '">'; 
							}
						}
					// Check file size
	...
	$empSQL="UPDATE employee SET  name='$name', gender='$gender', dob=$memdob, residence='$residence', prevemployer='$prevemployer', phone='$phone', idno=$natid, pin='$pin', narration='$narration', marital='$marital', pobox='$pobox',locationid='$loc',designation='$designation', nhif='$nssf', nssf='$nhif',idpic='$dbnameid',email='$email' WHERE empid='$empid'";
	...
	$empSQL="UPDATE employee SET  name='$name', gender='$gender', dob=$memdob, residence='$residence', prevemployer='$prevemployer', phone='$phone', idno=$natid, pin='$pin', narration='$narration', marital='$marital', pobox='$pobox',locationid='$loc',designation='$designation', nhif='$nssf', nssf='$nhif',picpath='$dbname',idpic='$dbnameid',email='$email' WHERE empid='$empid'";
