...
if($role>1){
	...
	if(!empty($_POST)){ 
		...
		$oldlnID=mysqli_real_escape_string($conn,$_POST['oldLnID']);
		$_SESSION['lnid']=$oldlnID;
	...
	VALUES('$lnNo','$custid','$login','$purpose',$loanamt,'$lword',$loantot,$period,$amtdeduct,'$securities',$secval,$servicecharge,$dtApp,$paydate,'D','n','np','$roid','$roid','$roid','$lntype','$tel','$prule',$other,$loanamt,$dedrule,$weeklyded)";
		...
		$RSloan=mysqli_query($conn,$loanSQL);
			  if(!$RSloan){
				echo $loanSQL." ". mysqli_error($conn);  
			 	...
			 	$_SESSION['newLnid']=$lnNo;
			echo '<h1 style="color: #0F0;">Amount successfully Deposited. </h1>';
			echo '<meta http-equiv="refresh" content="2;URL=../repeatg">';
