...
error_reporting(E_ALL);
  ...
  function loginById($user_id) {
    global $conn;
    $sql = "SELECT u.* ,r.name as role FROM users u LEFT JOIN roles r ON u.role_id=r.id WHERE u.id=? LIMIT 1";
    ...
    if (!empty($user)) {
      ...
      $_SESSION['user'] = $user;
        ...
        $name= htmlentities($_SESSION['user']['name']?? '');
        $id= htmlentities($_SESSION['user']['usercode']?? '');
        $uip=$_SERVER['REMOTE_ADDR']; // get the user ip
        $action="Login";
        // query for inser user log in to data base
        $admSQL="INSERT INTO userlog(userId,username,userIp,action) VALUES('".$id."','".$username."','$uip','$action')";
        mysqli_query($conn,$admSQL);
      ...
      if (isAdmin($user_id)) {
        ...
        echo '<h1 style="color: #0F0;">Login successful. Welcome '.$name.' </h1>';
