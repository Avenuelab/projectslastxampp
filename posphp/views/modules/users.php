    ...
    <div class="box">
      ...
      <div class="box-body">
        <table class="table table-bordered table-hover table-striped dt-responsive tables" width="100%">
          ...
          <tbody>
            <?php
              $users = []; // Assume this is fetched from the database
              // Example user data for demonstration purposes
              $users = [
                ["id" => 1, "name" => "John Doe", "user" => "johndoe", "photo" => "", "profile" => "Administrator", "status" => 1, "lastLogin" => "2023-10-11"],
              ...
              foreach ($users as $key => $value) {
                echo '<tr>
                        ...
                        <td>'.$value["name"].'</td>
                        <td>'.$value["user"].'</td>
                        <td><img src="'.($value["photo"] ?: "views/img/users/default/anonymous.png").'" class="img-thumbnail" width="40px"></td>
                        <td>'.$value["profile"].'</td>
                        <td>
                          <button class="btn btn-'.($value["status"] ? 'success' : 'danger').' btnActivate btn-xs" userId="'.$value["id"].'" userStatus="'.($value["status"] ? '0' : '1').'">'.($value["status"] ? 'Activated' : 'Deactivated').'</button>
                        </td>
          ...
          <div class="box-body">
            ...
            <div class="form-group">
              <div class="input-group">
                ...
                <input class="form-control input-lg" type="text" name="newUser" placeholder="Enter username" required>
              </div>
            </div>
            <div class="form-group">
              <div class="input-group">
...
<?php
  // Handle user deletion here (assuming deletion logic is implemented)
?>