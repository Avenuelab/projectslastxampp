...
{ 
    ...
    if($file!='')
    {
    $sql="SELECT * FROM banking WHERE id=$id limit 1";   
    $res=mysqli_query($conn,$sql);
        if($row=mysqli_fetch_array($res)) 
        {
            ...
            $sql="UPDATE banking SET name='$name',feature='$feature',pic='$image_file' WHERE id=$id";
            $result=mysqli_query($conn,$sql); 
    }
    else 
    {
            $result=mysqli_query($conn,"UPDATE banking SET name='$name',feature='$feature' WHERE id=$id");   
    } 
        if($result)
        {
            echo "<h1> Update was succesful</h1><br>";
            echo '<meta http-equiv="refresh" content="2;URL=../edit_bank.php">';
        }
        else 
        {
